const findThumbnailInContent = (
  thumbnailCheck: boolean,
  ...
  const checkThumbnail = thumbnailCheck ? titleBackgroundImage : content;
  const result = checkThumbnail.match(regex);
  if (result) {
    return result[0];
  }
